...
| Wager fairness — level bracket + creator approval | **DEFERRED to v5.2** | replace `accept_wager` with `request_accept_wager` + `approve_challenger` / `decline_challenger` / `withdraw_challenge` / `cancel_expired_challenge`; new `STATUS_PENDING_APPROVAL`; level-±1 snapshot on `WagerMatch`; abort codes 12-17. Full spec: [`docs/V5.2_WAGER_FAIRNESS_SPEC.md`](docs/V5.2_WAGER_FAIRNESS_SPEC.md) | Closes the high-level-farms-low-level abuse of first-come accept. UI-only gate is bypassable via direct PTB call — must be chain-enforced. Bundles into the v5.2 publish alongside attested settlement. |
