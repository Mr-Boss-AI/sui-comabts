...
| Wager fairness — level bracket + creator approval + referee-liveness escape hatch | **🛠️ IMPLEMENTED + TEST-COVERED (2026-05-30), PENDING TESTNET PUBLISH** | `contracts/sources/arena.move` + `contracts/tests/arena_tests.move` rewritten on `feature/v5.2-wager-fairness`. Replaces v5.1's first-come `accept_wager` with the full handshake (`request_accept_wager` + `approve_challenger` / `decline_challenger` / `withdraw_challenge` / `cancel_expired_challenge`); new `STATUS_PENDING_APPROVAL`; level-±1 snapshot on `WagerMatch`; **adds `reclaim_stalled_wager` participant escape hatch (30-min `WAGER_RESOLUTION_TIMEOUT_MS`) closing the centralized-referee single-point-of-failure**; abort codes 12-20 per spec + 21/22 judgment calls (`ECreatorFightLocked`, `ENotCharacterOwner`). Tests: **104 / 104 PASS** (was 71 v5.1; +33 net arena_tests, every code 12-20 directly triggered). Full spec: [`docs/V5.2_WAGER_FAIRNESS_SPEC.md`](docs/V5.2_WAGER_FAIRNESS_SPEC.md). | Closes the high-level-farms-low-level abuse of first-come accept AND the centralized-referee funds-in-limbo risk. UI-only gate is bypassable via direct PTB call — must be chain-enforced. **Scope locked to wager-fairness only** (NOT bundled with `settle_wager_attested` / `sui::random` / `respec_character` per 2026-05-30 user direction); those remain on the v5.2/v5.3 backlog. |
